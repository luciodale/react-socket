...
- **One primitive per concern.** Ten focused hooks. No message switch, no pub/sub layer, no `.on` / `.remove` anywhere in user code.
...
- **Production debug stream.** `onDebug` / `addDebugListener` ships every internal lifecycle event to Sentry, Datadog, or your own logger.
...
Browser WebSockets cannot carry an `Authorization` header. Four standard workarounds, all first-class:

```ts
// Dynamic URL — refreshed every (re)connect
const manager = new WebSocketManager<TClientMsg, TServerMsg>({
  url: async () => `wss://api.example.com/ws?token=${await getToken()}`,
  // ...
})

// First-message auth (recommended) — open unauthenticated, send auth as first frame
const manager = new WebSocketManager<TClientMsg, TServerMsg>({
...
See the [Authentication guide](https://koolcodez.com/projects/react-socket/docs/auth) for query string, subprotocol, cookie, and first-message recipes including mid-session re-auth.

## Binary frames

Opt in via the wire-type generic. Default stays `string`; widen to `ArrayBuffer`, `Blob`, or a union to send MessagePack, protobuf, or raw bytes.
...
const manager = new WebSocketManager<
  ...
  TServerMsg,
  "type",
  ArrayBuffer,  // outgoing wire type
  ArrayBuffer   // incoming wire type
>({
  url: "wss://...",
