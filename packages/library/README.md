<div align="center">
  ...
  <h1>react-socket</h1>
  <p>A TypeScript-first WebSocket manager for React. Five hooks, one way to do each thing, zero imperative glue.</p>
...
- **One primitive per concern.** Five hooks, each with a distinct job. No message switch, no pub/sub layer, no `.on` / `.remove` anywhere in user code.
